...
// FIX: Use `process.env.API_KEY` as required by the coding guidelines. This fixes the TypeScript error and aligns with best practices.
...
export const generateCardImage = async (): Promise<string> => {
  ...
  if (!ai) {
    // FIX: Updated error message to refer to the correct environment variable `API_KEY`.
